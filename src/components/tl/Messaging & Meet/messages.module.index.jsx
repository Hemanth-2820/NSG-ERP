...
export default function Messages({ initialSelectedChannel, currentUser }) {
              ...
              return {
                ...
                messages: msgs.map(m => {
                  ...
                  return {
                    ...
                    isMe: m.sender === tlName || m.sender === tlName + ' (TL)',
  ...
  useEffect(() => {
    ...
    const wsPath = isLocal ? `/employee-portal/ws/${encodeURIComponent(tlName)}` : `/api/employee-portal/ws/${encodeURIComponent(tlName)}`;
            ...
            return {
              ...
              messages: (c.messages || []).map(m => {
                ...
                try {
                  ...
                  return { ...m, seen_by: JSON.stringify([tlName]) };
                }
                return m;
              })
            };
  ...
  const handleEndCall = () => {
    ...
    if (!msgIdToUpdate) {
       ...
       const myCallMsgs = msgs.filter(m => (m.isCallStatus || m.attachment_type === 'call_status') && (m.isMe || m.sender === tlName));
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showDPMenu && (
                <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: '#FFF', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', border: '1px solid var(--ceo-border)', zIndex: 100, width: '150px', overflow: 'hidden' }}>
                  <button 
                    onClick={() => {
                      setLightboxMedia({ url: currentUser?.photo ? currentUser.photo : `https://ui-avatars.com/api/?name=${encodeURIComponent(tlName)}`, type: 'image' });
                      setShowDPMenu(false);
                    }}
