...
export function HrMessagingView({ initialSelectedChannel, currentUser }) {
  ...
  const myChannels = chatChannels.filter(c => !c.id.startsWith('dm-') && (c.id === "general-channel" || (c.members && c.members.includes('hr')))); // CEO has access to ALL channels
  ...
  useEffect(() => {
    ...
    if (selectedChannel && getUnreadCount(selectedChannel) > 0) {
      if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
        socketRef.current.send(JSON.stringify({ type: 'mark_read', channel_id: selectedChannel, user_name: userName }));
      }
          ...
          if (c.id === selectedChannel) {
            return {
              ...
              messages: (c.messages || []).map(m => {
                if (m.isMe || m.sender === userName) return m;
                try {
  ...
  const handleEndCall = () => {
    ...
    if (!msgIdToUpdate) {
       ...
       const myCallMsgs = msgs.filter(m => (m.isCallStatus || m.attachment_type === 'call_status') && (m.isMe || m.sender === userName));
    ...
    if (msgIdToUpdate) {
      try {
        const token = localStorage.getItem('nsg_jwt_token');
        fetch(`/api/hr-portal/chat/messages/${msgIdToUpdate}`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const getUnreadCount = (channelId) => {
    ...
    return msgs.filter(m => {
       if (m.isMe || m.sender === userName) return false;
       try {
                 ...
                 <div style={{ fontSize: '13px', color: '#92400E', maxHeight: '40px', overflowY: 'auto' }}>
                    {pinnedMsgs.filter(m => {
                      if (selectedChannel.startsWith('dm-')) {
                        ...
                        return m.sender === userName || m.sender === targetUser || m.sender === userName + ' (TL)' || m.sender === targetUser + ' (TL)' || m.sender === 'System';
                      }
                      return true;
                    }).map(pm => (
                       <div 
            ...
            <div style={{ position: 'relative', flex: 1 }}>
              <input 
                ...
                onChange={e => {
                  ...
                  if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
                    socketRef.current.send(JSON.stringify({ type: 'typing_start', channel_id: selectedChannel, sender: userName }));
                    if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
                    typingTimeoutRef.current = setTimeout(() => {
                      if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
                        socketRef.current.send(JSON.stringify({ type: 'typing_stop', channel_id: selectedChannel, sender: userName }));
                      }
                    }, 2000);
                  }
                }} 
           ...
           <div style={{ padding: '16px', borderTop: '1px solid var(--ceo-border)' }}>
             <input type="text" placeholder="Reply..." onKeyDown={e => {
                if (e.key === 'Enter' && e.target.value.trim()) {
                   const payload = { type: 'new_message', channel_id: selectedChannel, sender: userName, text: e.target.value.trim(), parent_id: activeThreadMessage.id };
                   if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {employees.filter(e => !(currentChannel.members || []).includes(String(e.id)) && !(currentChannel.members || []).includes(e.id)).map(emp => {
                    const avatar = emp.photo ? `${emp.photo}` : (emp.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(emp.name)}`);
                    return (
