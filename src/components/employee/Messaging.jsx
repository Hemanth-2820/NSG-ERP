...
export default function Messages({ initialSelectedChannel, currentUser }) {
  const empName = currentUser?.name || 'Michael Vance';
  ...
  const myChannels = chatChannels.filter(c => !c.id.startsWith('dm-') && (c.id === "general-channel" || (c.members && c.members.includes(String(currentUser?.id || 'emp')))));
  ...
  useEffect(() => {
    ...
    const wsPath = isLocal ? `/employee-portal/ws/${encodeURIComponent(empName)}` : `/api/employee-portal/ws/${encodeURIComponent(empName)}`;
            ...
            return {
              ...
              messages: (c.messages || []).map(m => {
                ...
                try {
                  ...
                  return { ...m, seen_by: JSON.stringify([empName]) };
                }
                return m;
              })
            };
  ...
  const handleStartCall = async () => {
    ...
    setHuddlePeer({
      ...
      roomName: isDM ? `DM-${getDmUser(selectedChannel)?.name}` : selectedChannel,
  ...
  const handleReactMessage = async (msgId, currentReactions, emoji) => {
    ...
    if(reactionsObj[emoji].includes(empName)) {
      reactionsObj[emoji] = reactionsObj[emoji].filter(n => n !== empName);
      if(reactionsObj[emoji].length === 0) delete reactionsObj[emoji];
    } else {
      reactionsObj[emoji].push(empName);
    }
                 ...
                 <div style={{ fontSize: '13px', color: '#92400E', maxHeight: '40px', overflowY: 'auto' }}>
                    {pinnedMsgs.filter(m => {
                      if (selectedChannel.startsWith('dm-')) {
                        ...
                        return m.sender === empName || m.sender === targetUser || m.sender === empName + ' (TL)' || m.sender === targetUser + ' (TL)' || m.sender === 'System';
                      }
                      return true;
                    }).map(pm => (
                       <div 
            ...
            <div style={{ position: 'relative', flex: 1 }}>
              <input 
                ...
                onChange={e => {
                  ...
                  if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
                    socketRef.current.send(JSON.stringify({ type: 'typing_start', channel_id: selectedChannel, sender: empName }));
                    if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
                    typingTimeoutRef.current = setTimeout(() => {
                      if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
                        socketRef.current.send(JSON.stringify({ type: 'typing_stop', channel_id: selectedChannel, sender: empName }));
                      }
                    }, 2000);
                  }
                }} 
          ...
          <div style={{ background: '#FFF', width: '400px', borderRadius: '12px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            ...
            <form onSubmit={handleCreateChannel} style={{ padding: '20px' }}>
              ...
              <div style={{ marginTop: '24px', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                <button type="button" onClick={() => { setIsCreateChannelOpen(false); setSelectedMembers([]); setNewChannelName(''); }} className="tk-confirm-btn" style={{ fontWeight: 700 }}>Cancel</button>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }} onClick={(e) => { if (e.target === e.currentTarget) { setIsEditChannelOpen(false) } }}>
          <div style={{ background: '#FFF', width: '400px', borderRadius: '12px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            ...
            <form onSubmit={handleEditChannel} style={{ padding: '20px' }}>
              ...
              <div style={{ marginTop: '24px', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                <button type="button" onClick={() => setIsEditChannelOpen(false)} className="tk-confirm-btn" style={{ fontWeight: 700 }}>Cancel</button>
      ...
      {contextMenu && (
        ...
        >
          ...
          >
            ...
            {(contextMenu.msg.isMe || (contextMenu.msg.sender && (contextMenu.msg.sender === empName || contextMenu.msg.sender.includes('Employee') || contextMenu.msg.sender.includes('CEO') || contextMenu.msg.sender.toLowerCase() === 'ceo' || contextMenu.msg.sender.includes('HR') || contextMenu.msg.sender.toLowerCase() === 'hr' || contextMenu.msg.sender.includes('TL') || contextMenu.msg.sender.toLowerCase() === 'tl'))) && (
              <>
                <button 
      ...
      {lightboxMedia && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {lightboxMedia.type === 'image' ? (
            <AvatarFallback  src={lightboxMedia.url} alt="fullscreen" style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain' }}  />
          ) : (
