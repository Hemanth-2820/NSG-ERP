...
export default function Messaging({ initialSelectedChannel, currentUser }) {
  ...
  const myChannels = chatChannels.filter(c => !c.id.startsWith('dm-') && (c.id === "general-channel" || (c.members && c.members.includes('ceo')))); // CEO has access to ALL channels
  ...
  useEffect(() => {
    ...
    const wsUrl = `${wsProtocol}//${wsHost}/employee-portal/ws/${encodeURIComponent(ceoName)}`;
  ...
  useEffect(() => {
    ...
    if (selectedChannel && getUnreadCount(selectedChannel) > 0) {
      if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
        socketRef.current.send(JSON.stringify({ type: 'mark_read', channel_id: selectedChannel, user_name: ceoName }));
      }
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showDPMenu && (
                <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', background: '#FFF', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', border: '1px solid var(--ceo-border)', zIndex: 100, width: '150px', overflow: 'hidden' }}>
                  <button 
                    onClick={() => {
                      setLightboxMedia({ url: currentUser?.photo ? currentUser.photo : `https://ui-avatars.com/api/?name=${encodeURIComponent(ceoName)}`, type: 'image' });
                      setShowDPMenu(false);
                    }}
      ...
      {contextMenu && (
        ...
        >
          ...
          >
            ...
            {(contextMenu.msg.isMe || (contextMenu.msg.sender && (contextMenu.msg.sender === ceoName || contextMenu.msg.sender.includes('CEO') || contextMenu.msg.sender.toLowerCase() === 'ceo' || contextMenu.msg.sender.includes('HR') || contextMenu.msg.sender.toLowerCase() === 'hr' || contextMenu.msg.sender.includes('TL') || contextMenu.msg.sender.toLowerCase() === 'tl'))) && (
              <>
                <button 
