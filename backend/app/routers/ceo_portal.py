...
def generate_mapped_pdf(
    ...
    for field_key, coords in mapping.items():
        if field_key == "signature":
            import base64
            sig_b64 = data.get("signature_base64", "")
        ...
        if field_key in values:
            ...
            if val:
                ...
                try:
                    ...
                    w = fitz.get_text_length(str(val), fontname="helv", fontsize=f_size)
                    rect = fitz.Rect(p.x, p.y - f_size, p.x + w, p.y + f_size * 0.2)
                    page.draw_rect(rect, color=(1,1,1), fill=(1,1,1))
                    page.insert_text(p, str(val), fontsize=f_size, fontname="helv", color=(0,0,0))
                except Exception as ex:
